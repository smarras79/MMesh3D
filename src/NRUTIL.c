 ...
 * increasing or decreasing. j=0 or j=n is returned to indicate that x
 * is out of range. */
...
//	printf("ythen = %f\n", *y);
	//printf("n = %d, Z = %f, y = %f, dErr = %f\n", n, x, *y, *dy);
	free_vector(d,1,n);
	free_vector(c,1,n);
}


void DERIVATIVES(int m, int n, float **dydx1, float **dydx2, float **d2ydx1dx2)
{
  /*This function is in support to point, polin2, and most of interpolation
   *functions because it is used to compute the derivatives 
   *dy/dx1,dy/dx2,d2y/(dx1*dx2) here stored in **dydx1,**dydx2,**d2ydx1dxw
   */
