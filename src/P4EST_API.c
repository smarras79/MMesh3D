...
/* THIS IS THE MAIN DRIVER FUNCTION CALLED FROM MOD_P4EST
   / * e.g. call p4esttonuma_init(is_cube, lnx, lny, lnz, &
...
/*void p8esttonuma_init(int *is_cube, int *nnx, int *nny, int *nnz, int *nref_levs,
		      int *read_external_grid_flg, int *is_non_conforming_flg,
		      int *xperiodic_flg, int *yperiodic_flg,
		      int *zperiodic_flg, int *mFACE_LEN,
		      int *lrestoring_sponge)*/

    void p8esttonuma_init(int is_cube, int nnx, int nny, int nnz, int nref_levs, \
		      int read_external_grid_flg, \
		      int xperiodic_flg, int yperiodic_flg, int zperiodic_flg)
		    
{
    ...
    */
    ...
    read_external = 1;
    is_non_conforming = 0;
    is_restoring = 0; //*lrestoring_sponge;
    ...
    /* Set refinement refinement flag 
       ...
       // Use do_refine array to set quadrant user_data, which holds refinemenr flags
                        ...
                        int *lrestoring_sponge)
{
    ...
    *p2n = malloc(sizeof(p4esttonuma_t));
    ...
    // for each point in each element (ngl*=nop*+1, nop: polynomial degree)
    ...
    (*p2n)->nboun = num_processor_boundary_faces //+num_physical_boundary_faces
